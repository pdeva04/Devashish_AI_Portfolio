...
import { Mail, Phone, MapPin, Send, Calendar, Settings } from "lucide-react";
...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailjsConfig.serviceId || !emailjsConfig.templateId || !emailjsConfig.publicKey) {
      toast.error("Please configure your EmailJS credentials first using the settings button.");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="project-card">
              ...
              <CardContent>
                <p className="text-muted-foreground">Amravati, India</p>
          ...
          <Card className="lg:col-span-2 project-card">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => setShowEmailjsConfig(!showEmailjsConfig)}>
            ...
            <CardContent>
              {showEmailjsConfig && <div className="mb-6 p-4 bg-muted/50 rounded-lg space-y-4">
                  <div>
                    <Label htmlFor="serviceId">EmailJS Service ID</Label>
                    <Input id="serviceId" value={emailjsConfig.serviceId} onChange={e => setEmailjsConfig(prev => ({
                  ...prev,
                  serviceId: e.target.value
                }))} placeholder="service_xxxxxxx" className="mt-2" />
                  </div>
                  <div>
                    <Label htmlFor="templateId">EmailJS Template ID</Label>
                    <Input id="templateId" value={emailjsConfig.templateId} onChange={e => setEmailjsConfig(prev => ({
                  ...prev,
                  templateId: e.target.value
                }))} placeholder="template_xxxxxxx" className="mt-2" />
                  </div>
                  <div>
                    <Label htmlFor="publicKey">EmailJS Public Key</Label>
                    <Input id="publicKey" value={emailjsConfig.publicKey} onChange={e => setEmailjsConfig(prev => ({
                  ...prev,
                  publicKey: e.target.value
                }))} placeholder="Your public key" className="mt-2" />
                  </div>
                  <div className="text-xs text-muted-foreground space-y-2">
                    <p>Get these from your EmailJS dashboard: Service ID from Email Services, Template ID from Email Templates, and Public Key from Account settings.</p>
                    <p><strong>Template Configuration:</strong></p>
                    <ul className="list-disc list-inside space-y-1 ml-2">
                      <li>From Email: Set to your verified email address (e.g., your.email@domain.com)</li>
                      <li>From Name: Use {"{{from_name}}"}</li>
                      <li>Reply-To: Use {"{{reply_to}}"}</li>
                      <li>Subject: Include relevant variables like {"{{project_type}}"}</li>
                    </ul>
                  </div>
                </div>}
